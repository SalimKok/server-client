...
def decrypt_message():
    ...
    # İstemciden anahtar beklemiyoruz, sadece şifreli metin geliyor.
    ...
    try:
        # --- BURASI DEĞİŞTİ ---
        # Server konsoluna uyarı düşecek ve input bekleyecek
        print(f"\n[!] YENİ MESAJ GELDİ ({method})")
        print(f"[!] Şifreli Metin: {ciphertext}")
        print("[?] Lütfen bu mesajı çözmek için Server terminaline anahtarı yazıp Enter'a basın...")
        
        # Server yöneticisi (sen) anahtarı buraya girecek:
        server_side_key = input("Anahtar: ")
        # ----------------------

        plaintext = cipher_strategy.decrypt(ciphertext, server_side_key)
        
        print(f"[OK] Mesaj çözüldü: {plaintext}\n")
...
if __name__ == '__main__':
    # use_reloader=False yapıyoruz ki input() komutu çift çalışıp hata vermesin

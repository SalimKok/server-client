...
class AESCipher(CipherInterface):
    ...
    def _key_expansion(self, key):
        ...
        while len(key_symbols) < 4 * self.nk:
            key_symbols.append(0) 
        ...
        for i in range(self.nk, self.nb * (self.nr + 1)):
            temp = w[i-1]
            if i % self.nk == 0:
                temp = [a ^ b for a, b in zip(self._sub_word(self._rot_word(temp)), 
                                              [R_CON[i // self.nk], 0, 0, 0])]
    ...
    def _add_round_key(self, state, w, round_num):
        for c in range(self.nb):
            for r in range(4):
                state[r][c] ^= w[round_num * self.nb + c][r]
        return state
    ...
    def _inv_mix_columns(self, state):
        for i in range(4):
            c = [state[x][i] for x in range(4)]
            state[0][i] = self._gmul(c[0], 0x0e) ^ self._gmul(c[1], 0x0b) ^ self._gmul(c[2], 0x0d) ^ self._gmul(c[3], 0x09)
            state[1][i] = self._gmul(c[0], 0x09) ^ self._gmul(c[1], 0x0e) ^ self._gmul(c[2], 0x0b) ^ self._gmul(c[3], 0x0d)
            state[2][i] = self._gmul(c[0], 0x0d) ^ self._gmul(c[1], 0x09) ^ self._gmul(c[2], 0x0e) ^ self._gmul(c[3], 0x0b)
